...
import Application
import State.Types
import State.Account (buildUser,mkUser)
...
requestShift :: User -> Shift -> Application (Maybe BS.ByteString)
...
getAvailableUsers :: Shift -> UserPlace -> Application [User]
getAvailableUsers shift place = do
  ...
  return $ mapMaybe ((fmap (\u -> u {uPlaces = [place]})) . mkUser . (flip buildUser [])) us
...
getShiftByRequest :: BS.ByteString -> Application (Maybe Shift)

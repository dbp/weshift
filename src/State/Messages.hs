...
getPlaceMessages :: UserPlace -> Int -> AppHandler [Message]
...
getPlaceNumMessages :: UserPlace -> AppHandler (Maybe Int)
...
addMessage :: UserPlace -> T.Text -> AppHandler Bool
...
voteMessageUp :: UserPlace -> User -> T.Text -> AppHandler Bool
voteMessageUp place user msgid = fmap (not.null) $ withPGDB "INSERT INTO message_ups (message, user_hash) (SELECT ?, encode(digest(?, 'sha1'),'hex') WHERE NOT EXISTS (SELECT message FROM message_ups WHERE message=? AND user_hash=encode(digest(?, 'sha1'), 'hex'))) RETURNING message;" [toSql msgid, toSql ident, toSql msgid, toSql ident]
  where ident = T.concat [msgid,"_",(uId user)]

voteMessageDown :: UserPlace -> User -> T.Text -> AppHandler Bool
...
flagMessage :: UserPlace -> User -> T.Text -> AppHandler Bool

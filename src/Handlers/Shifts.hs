...
import           Forms.Shifts
import           Mail                  (mailClaim, mailRequestOff,
                                        mailShiftCovered)
...
shiftAddH u p = do
  ...
  case result of
      ...
      Nothing -> do
        dayNum <- fmap (TE.decodeUtf8 . fromMaybe "") $ getParam "ws.day"
        heistLocal (bindSplices $ do "disp" ## textSplice "block"
                                     "dayNum" ## textSplice dayNum) $ heistLocal (bindDigestiveSplices view) $ renderWS "work/shift/add"
 where trd (_,_,a) = a
...
shiftEditH u p = do
    ...
    case result of
        Nothing -> do
          heistLocal (bindSplices $ do "disp" ## textSplice "block"
                                       "id" ## textSplice id') $ heistLocal (bindDigestiveSplices view) $ renderWS "work/shift/edit"
        Just (id', ShiftTime user start stop color units description) -> do
          ...
          case s of
            Nothing -> do
              dayNum <- fmap (TE.decodeUtf8 . fromMaybe "") $ getParam "ws.day"
              heistLocal (bindSplices $ do "id" ## textSplice id'
                                           "disp" ## textSplice "block"
                                           "message" ## textSplice "Could not find shift."
                                           "dayNum" ## textSplice dayNum) $ renderWS "work/shift/edit_error"
            Just shift -> do
...
shiftClaimH u p = do
  ...
  case result of
    Nothing -> heistLocal (bindSplices $ do "disp" ## textSplice "block"
                                            "id" ## textSplice id') $ heistLocal (bindDigestiveSplices view) $ renderWS "work/shift/claim"
        ...
        (Just shift, Just user) -> do
          ...
          case success of
            True -> do
              ...
              case target' of
                Nothing -> heistLocal (bindSplices $ do "id" ## textSplice id'
                                                        "disp" ## textSplice "block"
                                                        "message" ## textSplice "Error U. Contact help@weshift.org") $ renderWS "work/shift/claim_error"
                Just target -> do
                  emails <- getUserEmails target
                  mapM (\email -> mailClaim (uName target) (uName user) (emAddress email) (T.pack $ show units) reason (U.wsTimeStamp $ sStart shift)) emails
                  (day,daySplice) <- dayLargeSplices p u (toGregorian (localDay (sStart shift)))
                  heistLocal (bindSplices (daySplice <> (commonSplices day))) $ renderWS "work/month_day_large"
            False -> heistLocal (bindSplices $ do "id" ## textSplice id'
                                                  "disp" ## textSplice "block"
                                                  "message" ## textSplice "Error C. Contact help@weshift.org") $ renderWS "work/shift/claim_error"
        _ -> heistLocal (bindSplices $ do "id" ## textSplice id'
                                          "disp" ## textSplice "block"
                                          "message" ## textSplice "Could not find shift.") $ renderWS "work/shift/claim_error"

shiftClaimCancelH u p = do
  id' <- fmap (TE.decodeUtf8 . fromJust) $ getParam "id" -- if this isn't present, it means tampering, so don't care.
  claim' <- getShiftClaim id'
  case claim' of
    Nothing -> heistLocal (bindSplices $ do "id" ## textSplice id'
                                            "message" ## textSplice "Could not find claim.") $ renderWS "work/shift/claims_error"
    Just claim -> do
...
shiftSplitH u p = do
    ...
    case result of
        Nothing -> do
          heistLocal (bindSplices $ do "disp" ## textSplice "block"
                                       "id" ## textSplice id') $ heistLocal (bindDigestiveSplices view) $ renderWS "work/shift/split"
        Just (id', ShiftTime user start stop color units description) -> do
          let suser = if pFac p then user else (uId u)
          -- this is guaranteed to fail for non-facilitators editing other people's shifts
          s <- getUserShift suser id'
   ...
   where trd (_,_,a) = a
         retErr id' dayNum msg =
              heistLocal (bindSplices $ do "id" ## textSplice id'
                                           ...
                                           "dayNum" ## textSplice dayNum) $ renderWS "work/shift/split_error"



shiftDeleteH u p = do
  mid <- fmap TE.decodeUtf8 <$> getParam "shift"
  case mid of
    ...
    Just id' -> do
      ...
      case s of
        Nothing -> heistLocal (bindSplices $ do "id" ## textSplice id'
                                                "disp" ## textSplice "block"
                                                "message" ## textSplice "Could not find shift.")  $ renderWS "work/shift/delete_error"
        Just shift -> do
          deleteShift u shift
          (day,daySplice) <- dayLargeSplices p u (toGregorian (localDay (sStart shift)))
          heistLocal (bindSplices (daySplice <> (commonSplices day))) $ renderWS "work/month_day_large"
  ...
  case mid of
    ...
    Just id' -> do
      ...
      case s of
        Nothing -> heistLocal (bindSplices $ do "id" ## textSplice id'
                                                "disp" ## textSplice "block"
                                                "message" ## textSplice "Could not find shift.") $ renderWS "work/shift/deadline_done_error"
        Just shift -> do
...
requestOffH u p = do
 ...
 where err id' m = heistLocal (bindSplices $ do "id" ## textSplice id'
                                                "disp" ## textSplice "block"
                                                "message" ## textSplice m) $ renderWS "work/shift/request_error"

unRequestOffH u p = do
  mid <- fmap TE.decodeUtf8 <$> getParam "shift"
  mreq <- fmap TE.decodeUtf8 <$> getParam "reqid"
  case (mid,mreq) of
    (Just id',Just reqid) -> do
      ...
      case (r,s) of
        ...
        _ -> heistLocal (bindSplices $ do "id" ## textSplice id'
                                          "disp" ## textSplice "block"
                                          "message" ## textSplice "Could not find shift.") $ renderWS "work/shift/unrequest_error"

    _ -> redirPlaceHomeAsync

-- | this is the non-ajax handler that is accessible without being logged it, suitable for emailing
coverShiftH = do
  mutok <- fmap TE.decodeUtf8 <$> getParam "t" -- the user's token
  mreqid <- fmap TE.decodeUtf8 <$> getParam "r" -- the id of the shift request
...
coverH u p = do
  muid <- fmap TE.decodeUtf8 <$> getParam "user"
  mid <- fmap TE.decodeUtf8 <$> getParam "shift"
  mreq <- getParam "req"
  case (muid,mid,mreq) of
    ...
    _ -> redirPlaceHomeAsync
 where err i msg = heistLocal (bindSplices $ do "id" ## textSplice i
                                                "disp" ## textSplice "block"
                                                "message" ## textSplice msg) $ renderWS "work/shift/cover_error"

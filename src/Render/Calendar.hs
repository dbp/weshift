...
monthSplices u p curday day' =
  do "monthName" ## textSplice $ T.pack (formatTime defaultTimeLocale "%B %Y" curday)
...
dayLargeSplices place user (year, month, day) = do
   ...
   return (d,do "day" ## renderDay user Nothing [] $ formatDay year month shifts [] user (emptyDayFormat (Just day))
                "closeDays" ## mapSplices (\n -> runChildrenWithText ("dayNum" ## T.pack $ show n)) (filter (/= day) $ take (gregorianMonthLength year month) $ iterate (+1) 1)
                "start- ##alue" ## textSplice "9:00am"
                "stop- ##alue" ## textSplice "5:00pm"
                "start- ##rrors" ## blackHoleSplice
                "stop- ##rrors" ## blackHoleSplice
                "id- ##rrors" ## blackHoleSplice
...
renderDay u day' shifts (DayFormat num myshifts othershifts top start end bottom request) =
  runChildrenWith $ do "boxClasses" ## textSplice boxclasses
...
daySplices :: User -> UserPlace -> [User] -> [Shift] -> Day -> Splices (Splice AppHandler)
daySplices u p workers shifts day = do
...
workerDay self start ss us dl =
  runChildrenWith $ do "name" ## textSplice $ uName dl
    ...
    where buildSs s (offset,acc) = let start = timePeriod (localTimeOfDay (sStart s))
          ...
          sS (offset,len,classes,time,desc) =
            runChildrenWith $ do "offset" ## textSplice $ T.pack $ show offset
